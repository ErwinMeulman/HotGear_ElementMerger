		...
		if (elementIds.Count != 0)
		{
			...
			if (list.Count > 1)
			{
				...
				foreach (List<Element> item in combinations)
				{
					try
					{
						InstanceVoidCutUtils.AddInstanceVoidCut(document, item[0], item[1]);
						num++;
					}
					catch
					{
				...
				MessageBox.Show(num.ToString() + " Pairs Elements Successfully Cut.", "ElementMerger");
				val.Commit();
			}
